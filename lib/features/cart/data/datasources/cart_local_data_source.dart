...
class CartLocalDataSource extends ICartLocalDataSource   {
  ...
  Future<GetCartItemsModel> getCart() async {
    return GetCartItemsModel(items: []);
  }

  @override
  Future<void> addToCart(CartItemModel item) async {
    final index = _cart.indexWhere((element) => element.product.id == item.product.id);
    if (index != -1) {
  ...
  Future<void> updateQuantity(int productId, int quantity) async {
    final index = _cart.indexWhere((element) => element.product.id == productId);
    if (index != -1) {
      if (quantity <= 0) {
